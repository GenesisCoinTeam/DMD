...
int nStakeMinAge = 60 * 60 * 24 * 7;	// minimum age for coin age: 2d
int nStakeMaxAge = 60 * 60 * 24 * 30;	// stake age of full weight: 30d
int nStakeTargetSpacing = 60;			// 1-minute block spacing
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
    ...
    if(fTestNet && !fProofOfStake && pindexLast->nHeight <= 500)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int64 a = vtx[0].GetValueOut();
    int64 b = GetProofOfWorkReward(pindex->nHeight, nFees, prevHash) + GetDevCoin(totalCoin);
    if (a > b)
		return false;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    if(totalCoin >= VALUE_CHANGE)
    {
        if (IsProofOfStake() && (vtx[0].vout.size() != 1 || !vtx[0].vout[0].IsEmpty() || !vtx[0].vout[1].IsEmpty() ))
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMinAge = 15 * 60; // test net min age is 15 min
        nStakeMaxAge = 6000 * 60; // test net max age is 60 min
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (true)
    {
        ...
        if (vRecv.end() - pstart < nHeaderSize)
        {
            ...
            {
                printf("\n\nvRec = %d, nHeaderSize = %d \n\n",(int)vRecv.size(),nHeaderSize);
